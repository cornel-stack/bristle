...
> **First-run tour (slice 025, Tier-3.7 deferred → shipped as the Tier-4 CAPSTONE)**: a 5-step spotlight overlay auto-running on the dashboard for a first-time-this-session user (onboarding.pdf p.3) — palette → problem card → Saved → Alerts → a centered closing. **THE ONE NEW CLIENT ISLAND** (`components/app/tour/first-run-tour.tsx`), mounted dashboard-only in `app/app/page.tsx`; the 5 static steps live in a neutral `tour/tour-steps.ts` (the slice-015 `constants.ts` pattern — no reads). **A1 — ephemeral, session-scoped, a module-level `let tourDismissed` singleton** (NOT a `TourContext` — over-engineering for one boolean): seeded "not yet seen" via `useState(() => !tourDismissed)`, survives soft nav (the module stays loaded), resets on a hard reload (the module is re-evaluated). Dismiss (×/Esc/End-tour/backdrop/step-5-Done) flips the singleton. **No DB write, no server action, no localStorage AND no sessionStorage** (the Tier-4 ephemeral convention — 4.5/4.6/4.9 — evolved to dashboard-scope); **the no-`tour_completed`-column fact settles A1** — real-persist would force a migration this slice is scoped not to add AND still hide the tour from every reviewer after the shared-demo's first dismissal. **A2 — hand-rolled** (the 4.8 dependency rule): dim overlay + a spotlight cutout (`box-shadow: 0 0 0 9999px <dim>` via theme-aware `color-mix(var(--color-text-primary) 40%)`) + a positioned bubble + the shared dialog focus-trap (add-category-modal pattern) + a 5-step machine — **no new dep**; the uncertain part is bubble-vs-target positioning (`getBoundingClientRect` + reposition on resize/scroll), which is layout not a11y-correctness, so it doesn't clear the cmdk bar. **A3 — `data-tour` anchors on in-app wrappers ONLY**: the topbar ⌘K trigger (`command-palette-trigger.tsx`), the FIRST dashboard card's wrapping `Link` (`problem-grid.tsx`, index 0 — the shared `@bristle/ui ProblemCardFull` leaf is NEVER touched), and the Saved + Alerts `NAV` entries (`sidebar-nav.tsx`, via an optional `tourKey`). Missing target (empty dashboard / narrow-width hidden trigger) → that step centers the bubble, never blocks. **A4 — the save step (3) points at the Saved nav + teaches the S-shortcut in step 2's tip** (dashboard cards have no visible save button — save is keyboard/detail per 4.5/4.8 — so don't spotlight an affordance that isn't there). **No new `packages/db` helper** (the tour reads no data — it renders against the DOM already present), no schema/seed/migration, no new dep. **Follow-up TF-028** (Tier-5.5): the ephemeral flag → a real per-user `tour_completed` (the NEW column + migration this slice does not add), now covering 4.5/4.6/4.9 + the tour. **After this slice → Tier-4 complete + the capstone → tag v0.4.0.**

> **Python pipeline scaffold + HN ingester (slice 026 = build-plan 5.1, Tier-5 FOUNDATION)**: the first live-data slice — `apps/pipeline/` (Python 3.12 + FastAPI + Inngest, deployed on Railway) ingests Hacker News via the keyless Algolia HN Search API every 4h into a new `raw_items` table with **DB-enforced** dedup. **`apps/pipeline` stays OUT of the pnpm/Turbo workspace** (no `package.json`); tooling is **uv** (`pyproject.toml` + `uv.lock`), **ruff**, **pytest** — a **standalone `.github/workflows/pipeline-ci.yml`** (uv+ruff+pytest on an ephemeral `postgres:16` SERVICE, never dev/prod), NOT in the Turbo graph. **Schema (Decision 2)**: `raw_items` is **Drizzle-owned** (migration **`0005`**, additive — `CREATE TABLE raw_items` + `UNIQUE(content_hash)` + two btree indexes, touches NO app table); Python reads/writes it via **asyncpg** and never defines DDL. The TS↔Python seam is enforced by a **drift test** (`test_schema_contract`): live `information_schema` == committed `packages/db/contracts/raw_items.contract.json` == Python `RAW_ITEMS_COLUMNS`; `db:contract` regenerates the artifact and is chained into `db:generate` so it can't go stale. **Idempotency (Decision 4)**: dedup is a DB invariant — `content_hash` (sha256 of STABLE fields `source|source_id|title|url|body`, excluding volatile `points`/`num_comments` + all ingest metadata) `UNIQUE` + `INSERT … ON CONFLICT DO NOTHING`, so overlapping/retried/concurrent runs can't double-write (the conflicting row keeps first-fetch values — a tested freeze). **Stateless watermark = `max(source_created_at) − B`** (B=24h, NOT strict `>max`; OD-1) re-scans the lookback so late/out-of-order items are caught, with a 72h empty-table backfill. The Algolia **pagination cap** (~1000/query, `paginationLimitedTo`) is handled by **recursive half-open time-window subdivision** (split when `nbHits > 800` OR `exhaustiveNbHits` is false — margin below the hard cap), so a high-volume window is never silently truncated; ties at a seam land on exactly one side. Orchestration: Inngest cron `0 */4 * * *`, `concurrency:1`, `retries:3`. **Inngest handler convention (the production bug + guard)**: the SDK (0.5.18) invokes the handler with a **single context arg** (`async def hn_ingest(ctx)`, steps via `ctx.step`) — a two-arg `(ctx, step)` handler registers but fails at dispatch ("missing 1 required positional argument: 'step'"); `test_inngest_handler` now dispatches through `_handler` with a single ctx so a signature mismatch fails CI (the core-only tests missed it). **Dev/prod DB split (Decision 1)**: a **separate Supabase project for dev** — the existing project is canonical **prod** (keeps the 15 fixtures + demo user), a new project is **dev**; local `DATABASE_URL` + the web app re-point to dev, prod lives in Vercel. **Two-project tax**: `db:migrate:all` applies the chain to **BOTH** dev + prod, **pre-flighting a connection to each and aborting before any apply if either is unreachable** (can't half-apply). The pipeline's `DATABASE_URL` is the chosen DB per env (Railway → prod; local `apps/pipeline/.env` → dev); **no Algolia key** (keyless endpoint). **No tag at 5.1** — v0.5.0 is the Tier-5 capstone (5.10). **OD-7 — the Python-dependency principle (standing)**: the JS "no new dep / hand-roll widgets" rule does **NOT** transfer to the Python runtime — you don't hand-roll a Postgres driver. The pipeline's dependency discipline is **standard + minimal + justified-per-need**; the six sanctioned deps — **uv, pytest, ruff, httpx, inngest, asyncpg** — are the foundational floor, and each future pipeline dep (LLM SDK, embeddings client, HDBSCAN/numpy at 5.2–5.3) is its **own per-slice decision**, not covered by this sanction.

> **Filter classifier + embedder (slice 027 = build-plan 5.2, Tier-5)**: the first per-item-SPEND slice + the first DATA-QUALITY DoD. A separate Inngest cron `process-items` (`concurrency:1`, decoupled from the ingester — a stateless `NOT EXISTS` pickup) classifies each unprocessed `raw_item` keep/drop via **one Claude Haiku (`claude-haiku-4-5-20251001`) tool-use call** → a **5-way `label`** (complaint/bug/feature-request/wish/noise) + reason + confidence, and embeds kept items via **OpenAI `text-embedding-3-small` @ 1536** into a new **`processed_items`** table (migration **`0006`**, additive, HNSW cosine index). **Keep/drop is a GENERATED column** `kept = (label <> 'noise' OR forced_keep)` — the single DB-computed source of truth (a forced-keep item has `label='noise'` but must count as KEPT; `label != 'noise'` alone would wrongly drop it). Per item is **atomic** (`ON CONFLICT (raw_item_id) DO NOTHING`); a partial failure leaves the item unprocessed + retried. Cost guard: `MAX_ITEMS_PER_RUN=1000` + `DAILY_USD_CEILING=$20` (runaway tripwire, NOT $5) + graceful halt; **CI mocks both providers → never spends**. The two-table drift/contract mechanism + dual-target `db:migrate:all` carry over from 5.1. **THE DATA-QUALITY DoD finding**: HN `tags=story` is **~95% NOISE** for problem-discovery (front-page = news/launches/off-topic; real dev problems live in Ask HN + comments) — the classifier works, the SOURCE is noisy → **the 028 HN-source-expansion slice** (Ask HN + scoped comments + a `subtype` column). On the story-only gold set (180 items, founder-corrected) the `--live` gate gives **noise-drop ~99% (firm DoD PASS) but retention ~75-88% on only 8 gold-keeps** — the retention floor is **PROVISIONAL (5.2-OD-6 deferred)**, finalized against representative sources in 028 per 5.2-OD-7 (story-titles don't generalize to body-heavy comments/Ask HN). **TF-029 (poison-item dead-letter)**: a per-item failure currently leaves the item unprocessed → a *poison* item that always fails is retried every run forever; add a dead-letter / attempt-count so chronically-failing items are parked, not infinitely retried. **Two rubric gaps to fix in 028**: (1) a **domain gate** — "is this a software/developer problem?" — an "ADAS for motorcycles" item slipped through as a keep; (2) **unmet-need recognition** — "how do you all deal with X / is there a tool that…" is genuine signal (wish), NOT noise (a Parquet-metadata Ask HN was wrongly dropped). `FORCED_KEEP_BELOW=0.5` (a starting default; forced-keep items still embed). **Gate outcome — RATIFIED accept-with-caveat (founder)**: the `--live` gate PASSED the firm noise-drop DoD (99.4% ≥ 80%) but the **PROVISIONAL** retention floor failed (75% = 6 of 8 keeps, a noisy story-only denominator) — accepted as the expected 5.2-OD-7 artifact, finalized against representative sources in 028; the two misses map to the two rubric gaps above; audit trail in `eval/gold_set.jsonl` + `eval/run_gate.py`; no `FORCED_KEEP_BELOW` tweak before 028. **DEV backfill (Batch E — against DEV, not prod)**: the Anthropic Batch-API one-shot (`backfill.py`, 5.2-OD-8) drained 3524 dev `raw_items` → **230 kept (6.5%, all embedded in pgvector) / 3294 dropped; zero duplicate `raw_item_id`; app tables 15/8/1 untouched; ~$0.3**; the incremental processor is proven on real data (idempotent re-run = 0 processed), but the **live autonomous Inngest cron deploys POST-merge** (Railway deploys from main). **TF-030 — pipeline → PROD DB target**: the live ingester + processor currently target the **DEV** database (Railway-on-main was the deploy ORIGIN, not the DB target); **before 5.5 (fixtures→live swap), re-point both at the PROD database the web app reads from** — at the top of 5.5 or a tiny prerequisite slice. NOT 5.2's job. **No tag at 5.2.**
