...
| 4 | Idempotency/reliability | **Layered:** DB `UNIQUE(content_hash)` + `ON CONFLICT DO NOTHING` is the primary guarantee; Inngest cron `0 */4 * * *`, `concurrency:1`, retries; **stateless watermark = `max(source_created_at) − lookback buffer`** (NOT strict `>max`; buffer wider than Algolia's indexing lag, so late/out-of-order items are re-scanned and caught, not permanently skipped — dedup absorbs the overlap). No `ingest_state` table. Keyless Algolia HN; 429/5xx backoff. |
...
- **Late / out-of-order indexing (the watermark trap)** — Algolia can index an HN item *after* its `created_at` has been passed by a prior run. A strict `>max(source_created_at)` watermark would skip such an item **permanently**. Mitigation (Decision 4): the fetch lower bound is `max(source_created_at) − B` with `B` wider than Algolia's indexing lag, so every run re-scans the last `B` and catches late arrivals; content-hash dedup absorbs the re-scan. Watermark stays stateless.
- **Clock skew / watermark boundary** — same overlap mechanism covers boundary items; dedup makes the overlap harmless.
...
2. **Windowed re-fetch with deliberate overlap — lower bound = `max(source_created_at) − B`, not strict `>max`.** Each run fetches HN items with `created_at ≥ (max(source_created_at) in raw_items) − B`, where **`B` (the lookback buffer) is chosen wider than Algolia's worst-case indexing lag**. This is the crux: a strict `>max` watermark would **permanently skip** any item HN/Algolia indexes *after* we've already advanced past its `created_at` (late arrivals / out-of-order indexing); re-scanning the last `B` every run re-presents those items and **content-hash dedup absorbs everything already stored** at zero cost. The watermark stays **stateless** — derived from the data (`max(source_created_at)`), no stored cursor. *(On an empty table the run does a bounded initial backfill window instead. `B` is a tunable constant, not a secret.)*
...
**[SETTLED]: the layered model above**, with the DB unique constraint as the primary, non-negotiable guarantee and Inngest concurrency/retries as belt-and-suspenders. *Sub-question — [SETTLED]:* derive the watermark **statelessly from `max(source_created_at)` MINUS a lookback buffer `B`** (wider than Algolia's indexing lag) — re-scan the overlap each run so late/out-of-order items are caught, with dedup absorbing the re-scan. **Not** a strict `>max` (which would skip late arrivals forever). No `ingest_state` table at 5.1 (revisit if multi-source/cost telemetry needs a stored cursor later).
