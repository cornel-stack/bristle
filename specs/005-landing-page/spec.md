...
- **FR-013**: A new compact problem card MUST exist as a separate component (a sibling to the canonical card, NOT a size prop or variant of it). It MUST be non-interactive and server-rendered and contain zero hardcoded color values. Its layout is a **denser** version of the canonical card, matching the design's sample-row cards: it KEEPS a category pill (top-left), a **small inline sparkline** (top-right, not a separate "14-day mentions" row), a serif title (smaller than the canonical), and an **italic quote with a leading source avatar** (tighter — fewer lines — but present); and it REMOVES the canonical's separate large-sparkline/"14-day mentions" row, the bottom "{n} quotes · {n} sources · last {window}" meta line, and the "Open report →" link, with tighter padding (12–16px, less than the canonical 24px).
...
- **Compact card inputs**: nearly the full canonical contract, rendered denser — title, category (key → label + color), momentum, **sparkline series** (shown as a small inline element), **top-quote text + quote-source** (shown as a tighter italic quote with a leading avatar), source markers, last-seen timestamp, plus an optional wrapping `href`. The sparkline and quote ARE part of the contract; only the canonical's separate large-sparkline row, bottom meta line, and "Open report →" link are dropped.
...
- **SC-006**: The compact card is a new sibling component (not a prop/variant), with zero hardcoded colors and padding under 24px; it keeps a small inline sparkline and a tighter italic quote, and drops the canonical's separate large-sparkline row, bottom meta line, and "Open report →" link. *(AC-6)*
