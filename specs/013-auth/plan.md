...
**7 — `auth.ts` shape.** `NextAuth({ adapter: DrizzleAdapter(db, {...table map}), session: { strategy: "database", maxAge: 2592000 }, pages: { signIn: "/login", verifyRequest: "/signup/verify-email-sent" }, providers: [], callbacks: { session } })` exporting `{ handlers, signIn, signOut, auth }`. `session` callback copies `emailVerified` onto `session.user`. Cookie defaults (httpOnly/secure-in-prod/sameSite=lax) accepted. Full contract in `contracts/ui-and-data.md`.

> **CORRECTION (Batch D gate, integration smoke).** This decision originally specified `providers: [Credentials({ authorize })]`. That is **invalid in Auth.js v5**: `@auth/core` `assertConfig()` throws `UnsupportedStrategy` on *every* `auth()` call when a Credentials provider is the only provider **and** `session.strategy === "database"` (`assert.js`: `if (dbStrategy && onlyCredentials)`). It broke all authenticated requests — `auth()` threw, the session resolved to null, and protected pages looped back to `/login`. The credentials login flow never used Auth.js `signIn()` (it verifies the password and creates the DB session itself via the adapter — decision 14 / `lib/auth/session.ts`), so the `Credentials` provider was dead code. Resolution: `providers: []`. The DrizzleAdapter still backs database-session reads/writes; `signOut()` and `handlers` are unaffected. Deferred OAuth re-adds non-credentials providers later without re-tripping the rule (`onlyCredentials` becomes false). The original `authorize()` description (email lookup + argon2 `verifyPassword`, no email-verified gate) no longer applies.
