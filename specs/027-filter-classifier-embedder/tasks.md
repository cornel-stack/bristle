...
**User-story coverage**: US1 (label+embed, FR-001/002) → A+B+E · US2 (idempotent/incremental, FR-004/005) → T015/T019/T028 · US3 (no half-write, FR-006) → T016/T020 · US4 (cost cap, FR-010/012) → T017/T021/T022 + the backfill T037 · DoD (FR-009 two-sided) → Batch D.
...
- [ ] **T016** Extend `apps/pipeline/src/pipeline/inngest_fns.py` — a **SECOND** scheduled function `fn_id="process-items"` (distinct from `hn-ingest`; same app/service — 5.2-OD-4), **`concurrency:1` (like the 5.1 ingester — so cron runs never overlap)**, retries; single-context handler (the 5.1 convention); invokes `process.run`. (FR-008, Decision 4)
...
- [ ] **T020** [P] **`tests/test_atomic_partial_failure.py` (GATE)** — embed **fails after** classify for a kept item → **no `processed_items` row** → retried next run. **Plus a DB-level concurrency assertion**: two processor runs processing the **same `raw_item` concurrently** yield **exactly one** `processed_items` row via the `ON CONFLICT (raw_item_id)` path, **with no error raised** (pairs with T016's `concurrency:1`). (FR-006, SC-005)
...
- [ ] **T024** [P] **`tests/test_forced_keep.py`** — `confidence < FORCED_KEEP_BELOW` forces keep regardless of a `noise` label, the item **embeds**, and `forced_keep=true` + `confidence` are stored. (FR-011)
  **CHECKPOINT B (non-code gate)** — STOP for review: the 5.1 ingester untouched + the **full suite green on `pgvector/pgvector:pg16` with both providers mocked, ZERO spend**; **explicitly confirm the 5.1 ingester suite still passes on the `pgvector/pgvector:pg16` image** (the CI image flip must not regress 5.1). Do not start Batch C until reviewed.
...
- [ ] **T037** Create `apps/pipeline/src/pipeline/backfill.py` + a one-shot command — **Anthropic Message Batches**: submit the ~3,200 backlog, **poll to completion, write results through the SAME atomic `insert_processed` path** (5.2-OD-8). **Routes through the SAME `cost.py` spend guard as the incremental path — NOT a bypass**, so the daily ceiling protects it too (refinement 3). **Founder-run**, gated on keys. **MUST run to completion BEFORE the cron is enabled (T038)** — while the async batch is in flight the items still have no `processed_items` row, so a live cron would double-classify them (refinement 2). (FR-014, SC-006)
- [ ] **T038** **AFTER the T037 backfill has fully drained the backlog**, register/serve the **`process-items`** cron (same app/service as `hn-ingest`, distinct `fn_id`; 5.2-OD-4); a forced run then processes only genuinely-new live items → `processed_items` continues to populate on prod. (FR-008)
...
- **Batch-E ordering (refinement 2)** — the one-shot backfill (T037) must **complete before** the incremental cron (T038) is enabled: in-flight async-batch items have no `processed_items` row yet, so a concurrent live cron would double-classify them. Backfill → drain → then cron. The backfill also routes through the **same `cost.py` guard** (refinement 3) — not a bypass.
