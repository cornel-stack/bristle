...
- [ ] T001 [P] [US1] Create `apps/web/src/lib/app-user.ts` — `getAppUser(): Promise<User>`, the **single demo-vs-session seam** (v1.0 → `getUserByEmail("demo@bristle.dev")`; the demo-email literal lives ONLY here; Tier-5.5 one-line flip to the session user). Resolves WHICH user, not WHETHER. (dep: none)
...
- [ ] T006 [US1] Create `apps/web/src/app/(app)/layout.tsx` (server) — authoritative `await auth()` gate (no session → `redirect("/login?callbackUrl=/app")`); resolve `getAppUser()`; render `<AppSidebar>` + `<AppTopbar>` + `{children}` on `surface-canvas`; light/dark via existing tokens. (dep: T001, T004, T005)
